...
const USER_AGENT = 'imrobot-cli/test-agent (+https://github.com/leopechnicki/im_robot)'
...
function normalizeUrl(input: string): URL {
  let raw = input.trim()
  // If the input carries an explicit scheme, refuse anything that isn't http(s)
  // BEFORE we try to prepend `https://` (which would otherwise silently produce
  // e.g. `https://ftp://example.com` — a valid but nonsensical URL).
  const schemeMatch = raw.match(/^([a-z][a-z0-9+.-]*):/i)
  if (schemeMatch) {
    const scheme = schemeMatch[1].toLowerCase()
    if (scheme !== 'http' && scheme !== 'https') {
      throw new Error(`Only http(s) URLs are supported (got ${scheme}:)`)
    }
  } else {
    raw = 'https://' + raw
  }
 ...
 */
export async function probeDiscoveryDocument(target: URL): Promise<DiscoveryDocument | null> {
  const wellKnown = new URL('/.well-known/imrobot.json', target.origin).toString()
  try {
...
export function scanHtmlForSignals(html: string): ProbeSignal[] {
  ...
  const scriptRegex = /<script[^>]*(?:src=["'][^"']*imrobot[^"']*["']|imrobot)[^>]*>/i
...
export async function probeUrl(rawUrl: string): Promise<ProbeResult> {
  ...
  if (
    htmlSignals.some((s) => s.kind === 'script_reference' || s.kind === 'meta_tag') ||
  ...
  return {
    ...
    reason: 'No discovery doc, no challenge attribute, no imrobot script or meta detected',
    httpStatus: pageResult.status,
  }
}
...
export function formatProbeResult(result: ProbeResult): string {
  ...
  if (result.discoveryDoc) {
    ...
    lines.push(`    proofHeader: ${result.discoveryDoc.endpoints?.proofHeader}`)
    if (result.discoveryDoc.difficulties) {
      lines.push(`    difficulties: ${result.discoveryDoc.difficulties.join(', ')}`)
    }
  }
  lines.push('')
  return lines.join('\n')
...
export async function cmdTestAgent(url: string | undefined, options: { json?: boolean } = {}) {
  if (!url) {
