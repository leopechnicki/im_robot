...
describe("scanHtmlForSignals", () => {
  it("detects a data-imrobot-challenge attribute", () => {
    const html =
      '<html><body><div data-imrobot-challenge=\'{"id":"x"}\'></div></body></html>';
  ...
  it("detects an imrobot script tag by src", () => {
    const html =
      '<html><head><script src="https://cdn.example.com/imrobot.min.js"></script></head></html>';
    const signals = scanHtmlForSignals(html);
  ...
  it("detects a meta[name=imrobot] tag", () => {
    const html =
      '<html><head><meta name="imrobot" content="v1.0"></head></html>';
    const signals = scanHtmlForSignals(html);
...
describe("probeUrl", () => {
  ...
  it("returns YES when page embeds challenge attribute (no discovery)", async () => {
    ...
    const result = await probeUrl("https://example.com");
    expect(result.verdict).toBe("yes");
    expect(result.signals.some((s) => s.kind === "challenge_attribute")).toBe(
      true,
    );
  });
  ...
  it("returns NO when no signals are present", async () => {
    installFetchMock((url) => {
      ...
      return {
        status: 200,
        body: "<html><body><h1>Plain site</h1></body></html>",
      };
    });
