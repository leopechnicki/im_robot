...
// PR #129 replaced both with `%IMROBOT_VERSION%` placeholders and added a
// Vite plugin (`html-version-inject`) whose `transformIndexHtml` hook
// substitutes the placeholder from the root package.json at build/serve time.
...
//   3. The plugin actually substitutes the placeholder with the current
//      package.json version (loads the real config, invokes the hook).
// ---------------------------------------------------------------------------
...
describe('demo HTML version badges (regression for PR #126 follow-up)', () => {
  ...
  it('vite plugin substitutes %IMROBOT_VERSION% with the current package version', async () => {
    // Load the demo vite config and pull out the html-version-inject plugin.
    ...
    const injectPlugin = plugins.find(
      (p: unknown): p is { name: string; transformIndexHtml: (html: string) => string } =>
        !!p && typeof p === 'object' && (p as { name?: string }).name === 'html-version-inject',
    )
